...
int main() {
    ...
    for(size_t i = 0; i < data_len; i++) {
        ...
        int64_t cur_add = 0;
        for(size_t k = zero_index; k > 0; k--) {
            printf("%zu\n", k);
            printf("%ld\n", differences[k][differences_len[k+1]]);
            cur_add = differences[k+1][differences_len[k+2]];
            differences[k][differences_len[k+1]] += cur_add;
        }
        cur_add = differences[1][differences_len[2]];
        sum += (differences[0][differences_len[1]] + cur_add);
